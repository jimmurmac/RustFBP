    ...
    impl PassthroughNode {
        ...
        pub fn new() -> Self {
           let result =  PassthroughNode {
            ...
            result.node_data().is_configured.set_is_ready(true);
        ...
        fn make_node(json_str: &String) -> Nodes {
            ...
            if node_result.is_ok() {
                ...
                if node_option.is_some() {
                    let the_node = node_option.unwrap();
                    let node_item = Some(Arc::new(the_node.clone()));
                    fn_result = Nodes::PassthroughNode(node_item);
                    the_node.clone().start();
                }
            }
            fn_result
        }
    ...
    impl AppendNode {
        pub fn new() -> Self {
            let result = AppendNode {
                ...
                append_data: Arc::new(Mutex::new(None)),
            };
        ...
        pub fn set_append_data(&mut self, data: String) {
            *self.append_data.lock().unwrap().deref_mut() = Some(data);
            self.node_data().is_configured.set_is_ready(true);
        }
    }
        ...
        fn make_node(json_str: &String) -> Nodes {
            ...
            if node_result.is_ok() {
                ...
                if node_option.is_some() {
                    let the_node = node_option.unwrap();
                    let node_item = Some(Arc::new(the_node.clone()));
                    fn_result = Nodes::AppendNode(node_item);
                    the_node.clone().start();
                }
            }
            fn_result
        ...
        fn make_node(json_str: &String) -> Nodes {
            ...
            if node_result.is_ok() {
                ...
                if node_option.is_some() {
                    ...
                    let the_node = node_option.unwrap();
                    let node_item = Some(Arc::new(the_node.clone()));
                    fn_result = Nodes::LoggerNode(node_item);
                    the_node.clone().start();
                }
            }
            fn_result
