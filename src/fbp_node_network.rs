...
//! Box<FBPNodeContext> 
//! 
//! as one of their fields, the clone will still point to the
...
impl NodeNetworkItem {
    ...
    pub fn add_connection(&mut self, node_name: &str, key: Option<String>) {
        ...
        self.connections.add_node(node_name.to_owned(), Some(hash_key));
        ...
        if get_has_nodesenum() {
            {
                ...
                if ctx_op.is_none() {
                    info!("Failed to crate a {} node", self.name());
                    return None;
                }
    ...
    fn make_connection(
        ...
        if get_has_nodesenum() {
            ...
            if let Some(parent_node_op) = parent_node_op {
                let parent_node = parent_node_op;
                parent_node.add_receiver(&mut ctx.clone(), key);
            }
            net_config.add_node(ctx.clone());

            if !self.connections.node_network.is_empty() {
...
impl NodeNetwork {
    ...
    fn insert_node(&mut self, a_node: NodeNetworkItem, hash_key: String) {
        if self.node_network.get_mut(&hash_key).is_some() {
            ...
            let network: Vec<NodeNetworkItem> = vec![a_node.clone()];
    ...
    pub fn find_node_by_name(
        ...
        if let Some(op_my_vec) = op_my_vec {
            let my_vec = op_my_vec;
            for nni in my_vec {
...
///
/// NodeNetworkItem::create_node returns an Option<NetworkConfiguration>.  
///
/// This can be used to work with the network as a whole.
///
...
mod tests {
    ...
    impl FBPNodeTrait for LoggerNode {
        ...
        fn process_message(&mut self, msg: IIDMessage) -> Result<IIDMessage, Error> {
            if msg.payload().is_some()  && self.log_string_to_file(msg.clone().payload().as_ref().unwrap()).is_err() {
                return Err(Error::new(
                    ErrorKind::Other,
