...
// use crate::fbp_node_trait::set_do_join_for_node_id;
...
pub struct SenderWrapper(pub std::sync::mpsc::Sender<IIDMessage>);
...
/// multiple threads will be able to access the underlying Sender
#[derive(Clone, Serialize, Deserialize)]
...
/// multiple threads will be able to access the underlying Receiver
...
impl FBPNodeReceiver {
    ...
    pub fn try_recv(&self) -> Result<IIDMessage, TryRecvError> {
        self.receiver.get_arc().lock().unwrap().deref().try_recv()
    }
}
...
///         msg: IIDMessage) -> Result<IIDMessage, Error> { Ok(msg.clone()) }
///
...
/// node.  All FBP nodes must have a FBPNodeContext member.  
///
#[derive(Clone, Serialize, Deserialize)]
...
impl FBPNodeContext {
    ...
    ///     info!("The node is running");
    /// } else {
    ///     info!("The node is NOT running");
    /// }
    /// ```
    ...
    ///     info!("The node has stopped running");
    /// } else {
    ///     info!("The node is still running");
    /// }
    ///
    /// ```
    ...
    ///     info!("The node is fully configured");
    /// } else {
    ///     info!("The node needs to be configured");
    /// }
    ///
    /// ```
    ...
    ///     info!("The node has been suspended");
    /// } else {
    ///     info!("The node is NOT suspended");
    /// }
    ///
    /// ```
    ...
    pub fn add_receiver(&mut self, receiver: &mut FBPNodeContext, key: Option<String>) {
        ...
        trace!(
            "Addiung a receiver: {} for key {}",
            receiver.name(),
            hash_key.clone()
        );

        if self.output_vec.get_type().is_empty() {
            let vec_for_key: Vec<Box<FBPNodeContext>> = vec![Box::new(receiver.clone())];
            self.output_vec
                .get_type()
                .insert(hash_key.clone(), vec_for_key);
        } else if self.output_vec.get_type().get_mut(&hash_key).is_some() {
                self.output_vec
                    .get_type()
                    .get_mut(&hash_key)
                    .unwrap()
    ...
    /// // process_message(self: &mut Self, msg: IIDMessage) -> std::result::Result<IIDMessage, Error>;
    ...
    pub fn post_msg_to_group(&self, msg: IIDMessage, key: Option<String>) {
        ...
        // trace!("posting {:?} to {}", msg, hash_key.clone());
...
/*
impl Drop for FBPNodeContext {
    fn drop(&mut self) {
        let msg = IIDMessage::new(MessageType::Stop, None);
        self.post_msg(msg);
        self.set_node_is_running(false);
        let node_id = self.uuid();
        // info!("Calling drop on node {}", node_id.clone());
        set_do_join_for_node_id(node_id.clone(), true);
    }
}
*/
...
mod tests {
    ...
    impl LoggerNode {
        ...
        pub fn log_string_to_file(&self, data: &str) -> Result<(), Error> {
            if self.log_file_path.is_none() {
                return Err(Error::new(
                    ErrorKind::Other,
            ...
            let string_to_write = data.to_owned();
    ...
    impl FBPNodeTrait for LoggerNode {
        ...
        fn process_config(&mut self, msg: IIDMessage) -> std::result::Result<IIDMessage, Error> {
            if msg.msg_type() == MessageType::Config  &&  msg.payload().is_some() {
                    ...
                    ConfigMessageType::Field => {
                        if config_message.data().as_ref().is_some() {
                            ...
                            if config_str.to_string().contains(key_str) {
                                ...
                                let convert_result = serde_json::from_str::<Value>(json_str);
                                if let Ok(convert_result) = convert_result {
                                    let json_value: Value = convert_result;
        ...
        fn process_message(&mut self, msg: IIDMessage) -> Result<IIDMessage, Error> {
            if msg.payload().is_some() {
                let log_string = msg.clone().payload().as_ref().unwrap().clone();
