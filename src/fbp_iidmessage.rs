...
pub enum MessageType {
    ...
    Process,
    /// The stop message ensures the message handling thread is stopped.
    Stop,
...
impl StructDeserializer for IIDMessage {}
impl StructSerializer for IIDMessage {}
...
impl IIDMessage {
    ...
    pub fn new(msg_type: MessageType, payload: Option<String>) -> Self {
        ...
        let the_payload: Option<String> = match payload {
            None => {
                None
            },
            Some(pay) => {
                let mut new_payload = pay.clone();
                new_payload = new_payload.trim().to_owned();
                Some(new_payload)
            },
        };
    ...
    ///     MessageType::Stop => { /* Deal with a Stop Message */}
...
impl fmt::Display for IIDMessage {
    // This trait requires `fmt` with this exact signature.
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let result = write!(f, "IIDMessage {{");

       result?;

        let result = match self.msg_type {
            MessageType::Data => {
                write!(f, "\tmsg_type: Data")
            }
            MessageType::Config => {
                write!(f, "\tmsg_type: Config")
            }
            MessageType::Process => {
                write!(f, "\tmsg_type: Process")
            }
            MessageType::Stop => {
                write!(f, "\tmsg_type: Stop")
            }
            _ => {
                write!(f, "\tmsg_type: Invalid")
            }
        };

       result?;

        let result = match self.sub_msg_type {
            SubMessageType::Normal => {
                write!(f, "\tsub_msg_type: Normal")
            }
            SubMessageType::Reply => {
                write!(f, "\tsub_msg_type: Reply")
            }
        };
        ...
        let result = match &self.payload {
            ...
            Some(s) => {
                ...
                temp_string = temp_string.trim().to_owned();
                write!(f, "\tpayload: {}", temp_string.clone())
            }
        };
